...
### Deploy dashboard on Render (Docker)
- Use `render.yaml` (root) or manual setup:
  - Environment: Docker
  - Root Directory: `dashboard`
  - Dockerfile path: `Dockerfile`
  - Region: Frankfurt (or your choice)
  - Env vars: `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `REDIRECT_URI` (https://<your-app>.onrender.com/callback), `FLASK_SECRET_KEY`, plus optional `WEATHER_API_KEY`, `NASA_KEY`, `ICON_URL`.
```bash
# If using render.yaml in the repo root:
